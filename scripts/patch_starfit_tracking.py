...
.measurement-tracking-header h3 {
  color: var(--green-dark);
  font-size: 20px;
  margin: 0 0 6px;
}

.measurement-tracking-header p {
  color: var(--muted);
  font-size: 14px;
  line-height: 1.45;
  margin: 0;
...
.measurement-chart-title strong {
  color: var(--green-dark);
  font-size: 16px;
}

.measurement-chart-title span {
  color: var(--muted);
  font-size: 12px;
  font-weight: 800;
}
...
if "/* starfit-tracking-charts */" not in text:
    ...
    text = text.replace(marker, css + marker)
    text = text.replace(
...
if 'id="measurementTrackingCharts"' not in text:
    old_html = '''  <div class="measurements-list" id="measurementList">
    <div class="empty-state">Cargando mediciones Starfit...</div>
  </div>'''
    new_html = '''  <section class="measurement-tracking" id="measurementTrackingSection">
...
text = text.replace(
    '<span>Músculo esquelético</span><strong>${escapeHtml(item.skeletal_muscle_kg || "—")}%</strong>',
    '<span>Músculo esquelético %</span><strong>${escapeHtml(item.skeletal_muscle_kg || "—")}%</strong>',
)
...
function renderMetricChart(config, points) {
  if (!points.length) {
    return `
      <article class="measurement-chart-card">
        <div class="measurement-chart-title"><strong>${escapeHtml(config.title)}</strong><span>${escapeHtml(config.unit)}</span></div>
        <div class="measurement-chart-empty">Todavía no hay datos suficientes para este indicador.</div>
      </article>
    `;
  }
  ...
  return `
    <article class="measurement-chart-card">
      <div class="measurement-chart-title">
        ...
        <span>${escapeHtml(latest.value.toFixed(config.decimals).replace('.', ','))} ${escapeHtml(config.unit)}</span>
      </div>
      <svg class="measurement-chart-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Evolución de ${escapeHtml(config.title)}">
        <path class="measurement-chart-area" d="${area}"></path>
        <path class="measurement-chart-line" d="${line}"></path>
        ${coords.map(point => `
          <circle class="measurement-chart-point" cx="${point.x.toFixed(1)}" cy="${point.y.toFixed(1)}" r="5"></circle>
          <text class="measurement-chart-label" x="${point.x.toFixed(1)}" y="${(point.y - 10).toFixed(1)}" text-anchor="middle">${escapeHtml(point.value.toFixed(config.decimals).replace('.', ','))}</text>
          <text class="measurement-chart-date" x="${point.x.toFixed(1)}" y="${height - 14}" text-anchor="middle">${escapeHtml(point.label)}</text>
        `).join('')}
      </svg>
    </article>
  `;
...
function renderMeasurementTrackingCharts(measurements) {
  ...
  if (!measurements.length) {
    measurementTrackingCharts.innerHTML = `
      <div class="measurement-chart-empty">
        Todavía no hay mediciones suficientes para graficar evolución.
      </div>
    `;
    return;
  }
  ...
  measurementTrackingCharts.innerHTML = configs
    .map(config => renderMetricChart(config, metricPoints(measurements, config.key)))
    .join('');
}
'''
...
if text != original:
    ...
    print("admin/alumno.html patched")
else:
